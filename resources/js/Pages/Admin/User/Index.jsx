        ...
        <AuthenticatedLayout header="Data User">
            ...
            <div className="bg-white rounded-2xl shadow-sm shadow-gray-200/50 border border-gray-100 overflow-hidden">
                <div className="p-4 sm:p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    ...
                    <Link href={route('admin.users.create')} className="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition shadow-sm">
                        + Tambah User
                    </Link>
                </div>
                            ...
                            {users.data.map(user => (
                                <tr key={user.id} className="hover:bg-indigo-50/30 transition duration-150">
                                    <td className="px-6 py-4 font-semibold text-gray-800 flex items-center">
                                        <div className="h-8 w-8 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center mr-3">
                                            {user.name.charAt(0)}
                                        </div>
                                        {user.name}
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right space-x-2">
                                        <Link href={route('admin.users.edit', user.id)} className="text-indigo-600 hover:text-white hover:bg-indigo-600 transition-colors bg-indigo-50 border border-indigo-100 px-3 py-1.5 rounded-md font-medium text-xs uppercase tracking-wide inline-block">Edit</Link>
